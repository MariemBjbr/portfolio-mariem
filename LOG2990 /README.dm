...
Lancer npm ci (Clean Install) pour installer les versions exactes des dépendances du projet. Ceci est possiblement seulement si le fichier package-lock.json existe. Ce fichier vous est fourni dans le code de départ.
...
Client
Ce projet est un code de départ pour un site Web fait avec le cadriciel (framework) Angular. Il contient un ensemble de composants et services de base pour vous aider à démarrer votre projet. La page principale permet de communiquer avec le serveur pour obtenir et envoyer des données par HTTP. Une page utilisant Angular Material est également disponible pour vous aider à démarrer avec ce framework si vous voulez l'utiliser.
...
Serveur
Ce projet est un code de départ pour un serveur dynamique fait avec la librairie Express ou NestJS. Il contient un ensemble de routes et de services de base pour vous aider à démarrer votre projet. Notez que la version NestJS contient plus de fonctionnalités et de structure que la version Express puisqu'elle contient la version NestJS des exemples de SocketIO et MongoDB présentés dans le cours.

Choix du serveur
Vous avez le choix entre 2 manières et ensembles de technologies pour développer votre serveur. Le dossier /server contient un serveur utilisant NodeJS et la librairie Express. Le dossier /server-nestjs contient un serveur utilisant le cadriciel de développement NestJS qui se base sur NodeJS et Express, mais est architecturalement très similaire à Angular.
Vous devez choisir un des deux projets pour votre développement. Lisez bien la section Choix de serveur à utiliser dans le README du serveur NestJS pour avoir plus de détails sur les actions à effectuer selon votre choix.
...
Build : transpile le code TypeScript en JavaScript et copie les fichiers dans le répertoire /out.
...
Les tests se retrouvent dans les fichiers *.spec.ts dans le code source des deux projets. Le client utilise la librairie Jasmine et le serveur utilise Mocha, Chai, Sinon et Supertest.
...
Debugger
Il est possible d'attacher un debugger directement dans VSCode pour les 2 projets. Le fichier launch.json contient les 2 configurations.
Important : avant de pouvoir utiliser le debugger sur un projet, il faut que celui-ci soit déployé localement avec la commande npm start.
...
Cette configuration permet de lancer un pipeline de validations sur le projet en 3 étapes dans l'ordre suivant: install, lint et test. Si une de ses étapes échoue, le pipeline est marqué comme échoué et une notification sera visible sur GitLab. La seule exception est l'étape de lint qui ne bloque pas le pipeline si elle échoue, mais donne quand même un avertissement visuel.
Vous pouvez consulter la console de l'étape échouée pour plus de détails sur la raison de l'échec.
Le pipeline sera lancé suite aux 2 actions suivantes : lors d'un commit sur la branche master ou dans le cas d'une demande d'intégration : Merge Request (MR) entre 2 branches. Dans le cas d'une MR, chaque nouveau commit lancera un nouveau pipeline de validation.
...
Nous vous invitons fortement à consulter le fichier CONTRIBUTING.md pour plus de détails. Suivre ces directives nous aidera à maintenir un projet bien organisé et facile à comprendre pour tous les membres de l'équipe.
